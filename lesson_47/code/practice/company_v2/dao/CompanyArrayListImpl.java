...
public class CompanyArrayListImpl implements Company{
   ...
   // private int size; // текущее кол-во сотрудников в компании
    ...
    public CompanyArrayListImpl(int capacity){
        employees = new ArrayList<>(); // длину указывать не надо
        this.capacity = capacity; // это ограничение по количеству сотрудников в компании
    }
    //O(n)
    @Override
    public boolean addEmployee(Employee employee) {
       ...
       employees.add( employee ); // добавили в список (элемент списка)
       return true;
    }
// O(n)
    @Override
    public Employee removeEmployee(int id) {
        ...
        if(victim != null){ // а надо ли это проверить ?
...
// O(n)
    ...
    public double avgSalary() {
        return totalSales()/ employees.size();
    }
    // O(n)
    @Override
