...
public class ArrayMethods3 {
    ...
    public static int binarySearch(int[] arr, int num) {
        ...
        return -left - 1;// возвращает отрицательное число, если элемент не найден
    }
    ...
    public static int[] positiveNumbers(int[] arr) {
        ...
        int[] positiveArray = new int[positiveCount];// Создаем новый массив для положительных чисел
        int index = 0;// индекс для заполнения нового массива

        // Копируем положительные числа из исходного массива в новый массив
        for (int i = 0; i < arr.length; i++) {
            if (arr[i] > 0) { // Проверяем, является ли элемент положительным
                positiveArray[index] = arr[i];
                index++;// Если элемент положительный, добавляем его в новый массив
            }  // Перебираем каждый элемент исходного массива


        }

        return positiveArray;// Возвращаем массив, содержащий только положительные числа
    }
