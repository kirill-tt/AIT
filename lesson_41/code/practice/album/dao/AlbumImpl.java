...
public class AlbumImpl implements Album {
    ...
    public static Comparator<Photo> comparator = (p1, p2) -> {
        int res = p1.getDate().compareTo( p2.getDate() );
        return res != 0 ? res : Integer.compare( p1.getPhotoId(), p2.getPhotoId());// photoId fields
    };
    ...
    public Photo getPhotoFromAlbum(int photoId, int albumId) {
        Photo pattern = new Photo( photoId,albumId, null, null, null); // определили тот объект, с которым сравниваем
        for (int i = 0; i < size; i++) {
